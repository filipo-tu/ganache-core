...
> {
  ...
  public async storageRangeAt(
    ...
    const transactionHashBuffer = transactions[txIndex].hash();
    ...
    // create state trie using parent block
    const trie = new SecureTrie(
      this.#database.trie,
      parentBlock.header.stateRoot.toBuffer()
    );
    ...
    let txHashCurrentlyProcessing: string | Buffer = null;

    const beforeTxListener = (tx: Transaction) => {
      txHashCurrentlyProcessing = Data.from(tx.hash()).toString();
      if (txHashCurrentlyProcessing == transactionHash.toString()) {
        vm.on("step", stepListener);
      }
    };
